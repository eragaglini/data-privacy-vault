...
1. L'applicazione client invia un dato sensibile (es. email).

2. Il Vault cifra il dato e lo salva su Redis.

3. Il Vault restituisce un token casuale (UUID parziale) al client.

4. Il client salva solo il token nel proprio database, mantenendo il dato sensibile fuori dal proprio perimetro.
...
```
git clone https://github.com/eragaglini/data-privacy-vault.git
...
⚠️ Attenzione: Questo codice è un Proof of Concept (PoC).

Per un utilizzo in produzione, considerare:
